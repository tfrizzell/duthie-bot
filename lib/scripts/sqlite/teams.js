...
const stmt = {
    ...
    insertLeagueTeamMap: prepare('INSERT OR IGNORE INTO league_team_map (leagueId, teamId) VALUES (?, ?)'),
    insertTeam: prepare('INSERT OR IGNORE INTO teams (name, shortname) VALUES (?, ?)'),
    insertTeamMap: prepare('INSERT OR IGNORE INTO team_map (siteId, mappedTeamId, teamId) VALUES (?, ?, ?)'),
    ...
    (err, row) => {
        ...
        child_process.fork(child, [JSON.stringify({...JSON.parse(extraData), leagueId: leagueId})], {env: {CHILD: true}})
            ...
            .on('message', teams => {
                ...
                Promise.all(teams.map(team => new Promise(resolve => {
                    stmt.getTeamId.get([
