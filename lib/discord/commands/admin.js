...
const stmt = {
    add: db.prepare(`INSERT INTO guild_admins (guildId, memberId) VALUES (?, ?)`),
    remove: db.prepare(`DELETE FROM guild_admins WHERE guildId = ? AND memberId = ?`)
};

const addAdmin = ([guildId, memberId], callback) => stmt.add.run([guildId, memberId], function(err, ...args) {
    if (err) {
...
const removeAdmin = ([guildId, memberId], callback) => stmt.remove.run([guildId, memberId], function(err, ...args) {
    if (err) {
        if (err.code === 'SQLITE_BUSY') {
