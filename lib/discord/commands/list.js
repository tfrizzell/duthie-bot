...
const stmt = {
    getAdmins: db.prepare(`SELECT memberId FROM guild_admins WHERE guildId = ? AND memberId != ?`),
    getLeagues: db.prepare(`SELECT leagues.id, leagues.name, leagues.codename, sites.name AS siteName FROM leagues JOIN sites ON sites.id = leagues.siteId AND leagues.disabled = 0 WHERE UPPER(?) IN (UPPER(sites.id), UPPER(sites.siteId), UPPER(sites.name), '') ORDER BY leagues.name, sites.name`),
    getSites: db.prepare(`SELECT id, name FROM sites ORDER BY id, name`),
    getTeams: db.prepare(`SELECT teams.id, teams.name, teams.shortname, REPLACE(GROUP_CONCAT(DISTINCT leagues.name), ',', ', ') AS leagueNames, REPLACE(GROUP_CONCAT(DISTINCT sites.name), ',', ', ') AS siteNames FROM teams JOIN league_team_map ON league_team_map.teamId = teams.id JOIN leagues ON leagues.id = league_team_map.leagueId AND leagues.siteId = sites.id AND leagues.disabled = 0 JOIN team_map ON team_map.teamId = teams.id JOIN sites ON sites.id = team_map.siteId WHERE UPPER(?) IN (UPPER(sites.id), UPPER(sites.siteId), UPPER(sites.name), '') AND UPPER(?) IN (UPPER(leagues.id), UPPER(leagues.name), leagues.codename, '') GROUP BY teams.id ORDER BY teams.name, leagues.name, sites.name`),
    getWatchers: db.prepare(`SELECT leagues.name AS leagueName, IFNULL(teams.name, 'All Teams') AS teamName, watcher_types.name AS type, watchers.channelId AS channel, sites.name AS siteName FROM watchers JOIN guilds ON guilds.id = watchers.guildId AND guilds.archived IS NULL JOIN watcher_types ON watcher_types.id = watchers.typeId JOIN leagues ON leagues.id = watchers.leagueId LEFT JOIN teams ON teams.id = watchers.teamId JOIN sites ON sites.id = leagues.siteId WHERE watchers.archived IS NULL AND watchers.guildId = ? AND UPPER(?) IN (UPPER(watcher_types.id), UPPER(watcher_types.name), '') AND UPPER(?) IN (UPPER(sites.id), UPPER(sites.siteId), UPPER(sites.name), '') AND UPPER(?) IN (UPPER(leagues.id), UPPER(leagues.name), leagues.codename, '') AND UPPER(?) IN (UPPER(teams.id), UPPER(teams.name), UPPER(teams.shortname), teams.codename, teams.codeshortname, '') AND ? IN (watchers.channelId, '') GROUP BY watchers.id ORDER BY leagues.id, leagues.name, teams.id, teams.name, watcher_types.name, channelId, sites.id, sites.name`),
    getWatcherTypes: db.prepare(`SELECT id, name, description FROM watcher_types`)
};
...
module.exports.admins = module.exports.administrators = (message, command, ...args) => {
    ...
    stmt.getAdmins.all([message.guild.id, ownerId], (err, admins = []) => {
        if (err) {
            throw err;
        }
...
module.exports.leagues = (message, command, ...args) => {
    ...
    stmt.getLeagues.all([site], (err, leagues = []) => {
        if (err) {
            throw err;
        }
...
module.exports.teams = (message, command, ...args) => {
    ...
    stmt.getTeams.all([site, league], (err, teams = []) => {
        if (err) {
            throw err;
        }
...
module.exports.watchers = (message, command, ...args) => {
    ...
    stmt.getWatchers.all([message.guild.id, type, site, league, team, channel], (err, watchers = []) => {
        if (err) {
            throw err;
        }
...
module.exports.watcherTypes = (message, command, ...args) => {
    ...
    stmt.getWatcherTypes.all((err, types = []) => {
        if (err) {
            throw err;
        }
