...
module.exports.leagues = (message, command, ...args) => {
    ...
    db.all(`SELECT leagues.id, leagues.name, leagues.codename, sites.name AS siteName FROM leagues JOIN sites ON sites.id = leagues.siteId AND leagues.disabled = 0 WHERE UPPER(?) IN (UPPER(sites.id), UPPER(sites.siteId), UPPER(sites.name), '') ORDER BY leagues.name, sites.name`, [site],
        (err, leagues = []) => {
            if (err) {
                throw err;
            }
