...
const outputList = (headers, objs) => {
    ...
    return [
        ...
        `| ${headers.join(' | ')} |`,
        `--${separator}--`,
        ...rows.map(row => `| ${row.map((value, index) => !Number.isNaN(Number(value)) ? value.padStart(width[index]) : value.padEnd(width[index])).join(' | ')} |`),
        `--${separator}--`
    ].join('\n');
};
...
module.exports.admins = module.exports.administrators = (message, command, ...args) => {
    ...
    db.all(`SELECT memberId FROM guild_admins WHERE guildId = ? AND memberId != ?`, [message.guild.id, ownerId], (err, admins = []) => {
        if (err) {
            throw err;
        }
...
module.exports.leagues = (message, command, ...args) => {
    ...
    db.all(`SELECT leagues.id, leagues.name, leagues.codename, sites.name AS siteName FROM leagues JOIN sites ON sites.id = leagues.siteId AND leagues.disabled = 0 WHERE UPPER(?) IN (UPPER(sites.id), UPPER(sites.siteId), UPPER(sites.name), '') ORDER BY leagues.name, sites.name`, [site], (err, leagues = []) => {
        if (err) {
            throw err;
        }
...
module.exports.sites = (message, command, ...args) => {
    ...
    db.all(`SELECT id, name FROM sites ORDER BY id, name`, (err, sites = []) => {
        if (err) {
            throw err;
        }
...
module.exports.watchers = (message, command, ...args) => {
    ...
    if (channel && Number.isNaN(Number(channel))) {
...
module.exports.watcherTypes = (message, command, ...args) => {
    ...
    db.all(`SELECT id, name, description FROM watcher_types`, (err, types = []) => {
        if (err) {
            throw err;
        }
