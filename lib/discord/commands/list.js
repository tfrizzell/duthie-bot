...
module.exports = (message) => {
    ...
    if (module.exports[cmd[2]]) {
        return module.exports[cmd[2]](message);
    } else {
        return module.exports.help(message);
    }
};

module.exports.help = (message) => new Promise(resolve => {
    const msg = message.content.trim().replace(/^`(.*?)`$/, '$1');
    const cmd = msg.split(/\s+/);
  
    resolve(new CommandResponse({
        channel: (cmd[3] === 'me') ? message.author : message.channel,
        options: {
          code: 'vb',
          split: 'true',
        },
        content: `Hello ${utils.tagUser(message.author, message.guild)}! Here is what you need to know about \`${config.prefix} list\`:
