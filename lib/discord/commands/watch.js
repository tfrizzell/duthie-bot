...
const stmt = {
    getLeagueId: db.prepare(`SELECT id FROM leagues WHERE UPPER(?) IN (UPPER(id), UPPER(name), codename) LIMIT 1`),
    getTeamId: db.prepare(`SELECT teams.id FROM teams JOIN league_team_map ON league_team_map.teamId = teams.id WHERE UPPER(?) IN (UPPER(teams.id), UPPER(teams.name), UPPER(teams.shortname), teams.codename, teams.codeshortname) AND league_team_map.leagueId = ? LIMIT 1`),
    getWatcherTypes: db.prepare(`SELECT id FROM watcher_types WHERE UPPER(?) IN (UPPER(id), UPPER(name)) OR (UPPER(?) = 'ALL-NEWS' AND id NOT IN (3, 5)) OR (UPPER(?) = 'ALL')`),
    register: db.prepare(`INSERT INTO watchers (guildId, typeId, leagueId, teamId, channelId) VALUES (?, ?, ?, ?, ?)`),
    unarchive: db.prepare(`UPDATE watchers SET archived = NULL WHERE guildId = ? AND typeId = ? AND leagueId = ? AND IFNULL(teamId, '') = ? AND IFNULL(channelId, '') = ? AND archived IS NOT NULL`),
    unregister: db.prepare(`UPDATE watchers SET archived = ? WHERE guildId = ? AND typeId = ? AND ? IN (leagueId, '') AND ? IN (IFNULL(teamId, ''), '*') AND ? IN (IFNULL(channelId, ''), '') AND archived IS NULL`)
};

const registerWatcher = ([guildId, typeId, leagueId, teamId, channelId], callback) => 
    stmt.unarchive.run([guildId, typeId, leagueId, teamId || '', channelId || ''], function(err, ...args) {
        if (err) {
        ...
        stmt.register.run([guildId, typeId, leagueId, teamId, channelId], function(err, ...args) {
            if (err) {
                if (err.code === 'SQLITE_BUSY') {
                    return setTimeout(() => registerWatcher([guildId, typeId, leagueId, teamId, channelId], callback), 100);
                } else if (err.code !== 'SQLITE_CONSTRAINT') {
...
const unregisterWatcher = ([timestamp, guildId, typeId, leagueId, teamId, channelId], callback) => 
    stmt.unregister.run([timestamp, guildId, typeId, leagueId, teamId, channelId], function(err, ...args) {
        if (err) {
            if (err.code === 'SQLITE_BUSY') {
...
module.exports.register = module.exports.watch = (message, command, ...args) => {
    ...
    utils.isAdmin(message.author, message.guild)
        .then(() => {
            ...
            stmt.getWatcherTypes.all([type, type, type], (err, types = []) => {
                if (err) {
                    throw err;
                }
                ...
                stmt.getLeagueId.run([league], (err, {id: leagueId} = {}) => {
                    if (err) {
                        throw err;
                    }
                    ...
                    stmt.getTeamId.get([team, leagueId], (err, {id: teamId} = {}) => {
                        if (err) {
                            throw err;
                        }
    ...
    utils.isAdmin(message.author, message.guild)
        .then(() => {
            ...
            stmt.getWatcherTypes.all([type, type, type], (err, types = []) => {
                ...
                stmt.getLeagueId.get([league], (err, {id: leagueId = ''} = {}) => {
                    if (err) {
                        throw err;
                    }
