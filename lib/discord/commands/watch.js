        ...
        .then(() => {
            ...
            db.all(`SELECT id FROM watcher_types WHERE UPPER(?) IN (UPPER(id), UPPER(name)) OR (UPPER(?) = 'ALL-NEWS' AND id NOT IN (3, 5)) OR (UPPER(?) = 'ALL')`, [type, type, type], (err, types = []) => {
                ...
                db.get(`SELECT id FROM leagues WHERE UPPER(?) IN (UPPER(id), UPPER(name), codename) LIMIT 1`, [league], (err, {id: leagueId} = {}) => {
                    ...
                    db.get(`SELECT teams.id FROM teams JOIN league_team_map ON league_team_map.teamId = teams.id WHERE UPPER(?) IN (UPPER(teams.id), UPPER(teams.name), UPPER(teams.shortname), teams.codename, teams.codeshortname) AND league_team_map.leagueId = ? LIMIT 1`, [team, leagueId], (err, {id: teamId} = {}) => {
                        if (err) {
                            throw err;
                        }
