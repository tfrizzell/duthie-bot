...
module.exports.announce = (message, command, ...args) => {
    ...
    for (const [id, guild] of client.guilds) {
        ...
        if (channel) {
            utils.getDefaultChannel(guild).send(`**ANNOUNCEMENT:** ${msg}`);
        } else {
            logger.info(`Could not find default channel for guild ${guild.name}`);
        }
    }
...
module.exports.broadcast = (message, command, ...args) => {
    ...
    for (const [id, guild] of client.guilds) {
        ...
        if (channel) {
            utils.getDefaultChannel(guild).send(msg);
        } else {
            logger.info(`Could not find default channel for guild ${guild.name}`);
        }
    }
