...
client.on('message', message => {
    ...
    if (message.channel.type === 'voice') {
        logger.debug(`Discarding message ${message.content} from ${message.author.tag} on guild ${message.guild.name} (${message.guild.id}): received over voice channel`);
    ...
    if (typeof commands[command.name] !== 'function') {
        logger.debug(`Discarding message ${message.content} from ${message.author.tag} on guild ${message.guild.name} (${message.guild.id}): command '${command.name}' not found`);
