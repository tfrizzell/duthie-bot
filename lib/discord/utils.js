...
module.exports.getChannel = (guild, channel) => {
    ...
    if (!(channel instanceof Discord.GuildChannel) || !channel.permissionsFor(client.user).has(Discord.Permissions.FLAGS.SEND_MESSAGES)) {
...
module.exports.getDefaultChannel = guild => {
    ...
    return guild.channels.filter(channel => 
        (channel.type == 'text') && 
        channel.permissionsFor(client.user).has(Discord.Permissions.FLAGS.SEND_MESSAGES)
