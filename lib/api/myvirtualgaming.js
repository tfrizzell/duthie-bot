                ...
                switch (paneName) {
                    case 'closed-bids':
                        while (item = reNews[paneName].exec(pane[2])) {
                            items.push({
                                id: helper.news.generateHash({ message: [item[3], item[4]].join(0x1F), teams: [teams[item[1]]], timestamp: item[5] }),
                                message: this.normalize(helper.news.cleanMessage(item[2])),
                                teams: [item[1]].filter(team => !Number.isNaN(Number(team)) && team > 0).map(team => parseInt(team)).sort(),
                                timestamp: moment.tz((item[5] || '').trim(), timezone).toISOString(),
                                type: 'bid'
                            });
                        }
                        break;
