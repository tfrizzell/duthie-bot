...
class MyVirtualGaming_API extends API {
    constructor(options = {}) {
        ...
        this.subdomain = options.subdomain || 'vghl';
    }
    ...
    buildUrl({
        ...
        return `https://${subdomain}.myvirtualgaming.com/vghlleagues/${league}/${path}?${this.buildQueryString(params)}`.replace(/\?+$/, '');
    }
    ...
    getInfo({leagueId}) {
        return new Promise((resolve, reject) => {
            this.get({
                ...
                resolve({
                    codename: $static.codename[leagueId] || json.feed.title._value.replace(/^(\S+).*/, '$1').trim(),
                    id: leagueId,
...
module.exports.new = (...options) => new MyVirtualGaming_API(...options);
