            ...
            this.get({
                ...
                while (week = reWeeks.exec(html)) {
                    promises.push(this.get({
                        ...
                        const reGame = /<div(?:[^>]+)?mvg-table(?:[^>]+)?>.*?<div(?:[^>]+)?schedule-team-logo(?:[^>]+)?><img(?:[^>]+)?\/(\w+)\.[^\.]+(?:[^>]+)?><\/div>.*?<div(?:[^>]+)?schedule-team-score(?:[^>]+)?>(.*?)<\/div>.*?<div(?:[^>]+)?schedule-team-logo(?:[^>]+)?><img(?:[^>]+)?\/(\w+)\.[^\.]+(?:[^>]+)?><\/div>.*?<div(?:[^>]+)?schedule-team-score(?:[^>]+)?>(.*?)<\/div>.*?<div(?:[^>]+)?schedule-summary-link(?:[^>]+)?><a(?:[^>]+)?&(?:amp;)?id=(\d+)(?:[^>]+)?>.*?<\/a><\/div>/i;
                        ...
                        while (line = reDateOrGame.exec(html)) {
                            ...
                            if (data = reGame.exec(line)) {
                                games.push({
                                    ...
                                    visitor: {
                                        id: data[1].trim(),
                                        score: !Number.isNaN(Number(data[2])) ? parseInt(data[2]) : null
                                    },
                                    home: {
                                        id: data[3].trim(),
                                        score: !Number.isNaN(Number(data[4])) ? parseInt(data[4]) : null
                                    }
                                });
                            }
                        }
