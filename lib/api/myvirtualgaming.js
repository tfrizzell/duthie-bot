...
class MyVirtualGaming_API extends API {
    ...
    getGames({leagueId, seasonId}) {
        ...
        return Promise.all([
            ...
            this.get({
                league: leagueId,
                path: 'schedule',
                ...(seasonId ? { single_seasons: seasonId } : {})
            }).then(html => {
                ...
                while (week = reWeeks.exec(html)) {
                    ...
                    promises.push(this.get({
                        ...
                        const reGame = /<div[^>]*game_div_\d+[^>]*><div[^>]*><div[^>]*><div[^>]*><div[^>]*\bschedule-team-logo\b[^>]*><img[^>]*\/(\w+)\.[^\.]+[^>]*><\/div><div[^>]*\bschedule-team\b[^>]*><div[^>]*\bschedule-team-name\b[^>]*>[^<]+<\/div><div[^>]*\bschedule-team-record\b[^>]*>[^<]+<\/div><\/div><div[^>]*\bschedule-team-score\b[^>]*>([^<]+)<\/div><\/div><div[^>]*><div[^>]*\bschedule-team-logo\b[^>]*><img[^>]*\/(\w+)\.[^\.]+[^>]*><\/div><div[^>]*\bschedule-team\b[^>]*><div[^>]*\bschedule-team-name\b[^>]*>[^<]+<\/div><div[^>]*\bschedule-team-record\b[^>]*>[^<]+<\/div><\/div><div[^>]*\bschedule-team-score\b[^>]*>([^<]+)<\/div><\/div><div[^>]*>\s*Matchmaking ID:\s*<strong[^>]*>\s*\d+\s*<\/strong>\s*<\/div><\/div><div[^>]*\bschedule-summary-link\b[^>]*><a[^>]*&(?:amp;)?id=(\d+)[^>]*>[^<]+<\/a><\/div><\/div><\/div><\/div>/i;
        ...
        return Promise.all([
            this.get({
                ...
                if (!json.feed.id.match(`/vghlleagues/${leagueId}/${leagueId}`)) {
                    throw new Error('Failed to get league info: feed did not match requested league');
                }
            ...
            new Promise((resolve, reject) => {
                this.get({
                    ...
                    const seasons = (html.match(/<select[^>]*\bsingle_seasons\b[^>]*>(.*?)<\/select>/) || ['', ''])[1].match(/<option(?=[^>]*selected)[^>]*value=["']?(\d+)["']?[^>]*>/g).map(opt => opt.replace(/.*?value=["']?(\d+)["']?.*/, '$1'));
                    resolve({ seasonId: seasons[seasons.length - 1] });
                }).catch(err => {
                    if (!(err instanceof API.Error)) {
                        return reject(err);
                    }

                    const seasons = (err.content.match(/<select[^>]*\bsingle_seasons\b[^>]*>(.*?)<\/select>/) || ['', ''])[1].match(/<option(?=[^>]*selected)[^>]*value=["']?(\d+)["']?[^>]*>/g).map(opt => opt.replace(/.*?value=["']?(\d+)["']?.*/, '$1'));
            ...
            new Promise((resolve, reject) => {
                this.get({
                    ...
                    const schedules = (html.match(/<select[^>]*\bfilter_schedule\b[^>]*>(.*?)<\/select>/) || ['', ''])[1].match(/<option[^>]*value=["']?(\d+)["']?[^>]*>/g).map(opt => opt.replace(/.*?value=["']?(\d+)["']?.*/, '$1'));
                    resolve({ scheduleId: schedules[schedules.length - 1] });
                }).catch(err => {
                    if (!(err instanceof API.Error)) {
                        return reject(err);
                    }

                    const schedules = (err.content.match(/<select[^>]*\bfilter_schedule\b[^>]*>(.*?)<\/select>/) || ['', ''])[1].match(/<option[^>]*value=["']?(\d+)["']?[^>]*>/g).map(opt => opt.replace(/.*?value=["']?(\d+)["']?.*/, '$1'));
    ...
    getNews({leagueId, scheduleId}) {
        if (!leagueId) {
            throw new TypeError(`leagueId is not defined`);
        }

        return Promise.all([
            this.getTeamMap({leagueId, scheduleId}),
    ...
    getTeamMap({leagueId, scheduleId}) {
        if (!leagueId) {
            throw new TypeError(`leagueId is not defined`);
        }
        ...
        return this.get({
            league: leagueId,
            path: 'rosters',
            ...(scheduleId ? {filter_schedule: scheduleId} : undefined)
        }).then(html => {
