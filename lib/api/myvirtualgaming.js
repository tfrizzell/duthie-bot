...
class MyVirtualGaming_API extends API {
            ...
            this.get({
                ...
                this.get({
                    ...
                    while (team = reTeams.exec(html)) {
                        const id = parseInt(team[1]);
                        teams[id] = {id: id, ...teams[id], shortname: this.normalize(({...teams[id]}.shortname || '').replace(team[2], '').trim() || team[2])};
                    }
    ...
    normalize(string) {
        return super.normalize(string).replace(/Bellevile Senators/, 'Belleville Senators');
    }
}
