...
const helper = {
    news: {
        ...
        generateHash: (item = []) => {
            const buf = [
                helper.news.cleanMessage(item[4].replace(/<span class="newsfeed_atn">(.*?)<\/span>/g, () => `::user::`)).replace(/::(team\d+)=.*?::/g, (a, b) => `::${b}::`),
            ...
            this.get({
                ...
                while (item = reNews.exec(html)) {
                    item = {
                        ...
                        message: helper.news.cleanMessage(item[4]),
                        teams: [...new Set([item[1], item[3], ...helper.news.getTeams(item[4])])].filter(team => !isNaN(team)).map(team => parseInt(team)).sort(),
