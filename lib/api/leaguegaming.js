...
const helper = {
    news: {
        ...
        generateHash: (item = []) => {
            const buf = [
                helper.news.cleanMessage(item[4]),//.replace(/<span class="newsfeed_atn">(.*?)<\/span>/g, '$1')),
...
class LeagueGaming_API extends API {
    ...
    getDailyStars({forumId, date = moment().subtract(1, 'days')}) {
        if (!forumId) {
        ...
        return new Promise((resolve, reject) => {
            this.get({
                path: 'search/1/',
                q: moment(date).format('[Daily 3 Stars For] dddd MMMM Do, YYYY'),
                o: 'date',
                'c[title_only]': 1,
                'c[node]': forumId
            }).then(html => {
                                ...
                                ...(stars[group] || []),
                                {
                                    ...
                                    metadata:  (group !== 'goalies')
                                        ? {
                                            ...
                                            'SV%': parseFloat(metadata[0]) / 100,
                                            'GAA': parseFloat(metadata[1]),
