        ...
        return new Promise((resolve, reject) => {
            this.get({
                ...
                while (item = reNews.exec(html)) {
                    item = {
                        id: getNewsItemHash(item),
                        message: cleanNewsItemMessage(item[4]),
                        teams: [...new Set([item[1], item[3], ...getNewsItemTeamsFromMessage(item[4])])].filter(team => !isNaN(team)).map(team => parseInt(team)).sort(),
                        timestamp: moment((item[5] || '').trim(), 'YYYY-MM-DD hh:mma').format(this.dateFormat),
                        type: (item[2] || '').trim()
                    };
        ...
        return new Promise((resolve, reject) => {
            this.get({
                ...
                while (team = reShortnames.exec(html)) {
                    const id = parseInt(team[1]);
                    teams[id] = {id: id, ...teams[id], shortname: team[2].trim()};
                }
...
const cleanNewsItemMessage = (message = '') => 
    message
        .replace(/<img(?:[^>]+)?\/team(\d+)\.png(?:[^>]+)?><span(?:[^>]+)?>(.*?)<\/span>/g, (a, b, c) => `::team${b}=${c}::`)
        .replace(/<\/span><span /g, '</span> <span ')
        .replace(/<[^>]+>/g, '')
        .replace(/[ ]+/g, ' ');

const getNewsItemHash = (item = []) => {
    const buf = [
        cleanNewsItemMessage(item[4].replace(/<span class="newsfeed_atn">(.*?)<\/span>/g, () => `::user::`)).replace(/::(team\d+)=.*?::/g, (a, b) => `::${b}::`),
        getNewsItemTeamsFromMessage(item[4]),
...
const getNewsItemTeamsFromMessage = (message = '') => 
    (message.match(/<img(?:[^>]+)?\/team(\d+)\.png(?:[^>]+)?><span(?:[^>]+)?>(.*?)<\/span>/g) || []).map(t => t.replace(/.*?team(\d+).*/, '$1'));

module.exports = LeagueGaming_API;
module.exports.new = (...options) => new LeagueGaming_API(...options);
