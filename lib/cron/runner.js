...
const stmt = {
    deleteArchivedGuilds: db.prepare(`DELETE FROM guilds WHERE archived <= ?`),
    deleteArchivedWatchers: db.prepare(`DELETE FROM watchers WHERE archived <= ?`),
    getDailyStars: db.prepare(`SELECT * FROM daily_stars WHERE queued = 1`),
    getGames: db.prepare(`SELECT * FROM games WHERE queued = 1`),
    getGameWatchers: db.prepare(`SELECT watchers.* FROM watchers JOIN leagues ON leagues.id = watchers.leagueId AND leagues.disabled = 0 JOIN guilds ON guilds.id = watchers.guildId AND guilds.archived IS NULL WHERE watchers.archived IS NULL AND watchers.typeId IN (5) AND watchers.leagueId = ? AND (watchers.teamId IS NULL OR watchers.teamId IN (?, ?)) GROUP BY watchers.guildId, watchers.channelId`),
    getNews: db.prepare(`SELECT * FROM news WHERE queued = 1`),
    updateDailyStars: db.prepare(`UPDATE data_daily_stars SET queued = 0 WHERE leagueId = ?`),
    updateGame: db.prepare(`UPDATE data_games SET queued = 0 WHERE id = ?`),
    updateNews: db.prepare(`UPDATE data_news SET queued = 0 WHERE id = ?`)
};
...
class CronRunner {
    ...
    processDailyStars() {
        ...
        db.serialize(() => {
            ...
            stmt.getDailyStars.all((err, rows) => {
                if (err) {
            ...
            stmt.getGames.all((err, games = []) => {
                ...
                Promise.all(games.map(game => 
                    new Promise(resolve => {
                        ...
                        stmt.getGameWatchers.all([game.leagueId, visitor.id, home.id], (err, rows = []) => {
                                if (err) {
                                    logger.error(err);
                                }
    ...
    removeArchivedData() {
        ...
        db.serialize(() => {
            ...
            stmt.deleteArchivedGuilds.run([timestamp], function(err) {
                if (err) {
            ...
            stmt.deleteArchivedWatchers.run([timestamp], function(err) {
                if (err) {
