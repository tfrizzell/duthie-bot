...
class CronRunner {
        ...
        db.serialize(() => {
            ...
            db.all('SELECT * FROM daily_stars WHERE queued = 1',
                (err, rows) => {
                    ...
                    Promise.all(Object.entries(stars).map(([leagueId, date]) => new Promise(resolve => {
                                ...
                                Promise.all(rows.map(row => new Promise(resolve => {
                                    ...
                                    for (const [group, stars] of Object.entries(data.stars)) {
                                        ...
                                        for (const star of stars) {
                                            ...
                                            if (!row.teamId || row.teamId === star.teamId) {
                                                buf.push(`    * ${utils.tagUser(guild, star.playerName)} - ${moment({d: star.rank}).format('Do')} Star ${group.replace(/^\./, a => a.toUpperCase())} __(${Object.entries(JSON.parse(star.metadata)).map(([key, val]) => `${val} ${key !== '+/-' ? key : ''}`.trim()).join(', ')})__`);
                                            }
                                        }
                                    }
                                    ...
                                    if (row === rows[0]) {
                                        logger.info(`[${leagueName}] ${buf.join('\n\t\t')}`);
                                    }
        ...
        db.serialize(() => {
            ...
            db.all('SELECT * FROM games WHERE queued = 1',
                (err, games = []) => {
                    ...
                    Promise.all(games.map(game => new Promise(resolve => {
                        const visitor = {
                            id: game.visitorTeam,
    ...
    updateLeagues() {
        child_process.fork(`${__dirname}/../scripts/sqlite/leagues.js`, {env: {CHILD: true}});
    }
