                                ...
                                (err, rows = []) => {
                                    ...
                                    Promise.all(rows.map(row => 
                                        new Promise(resolve => {
                                            ...
                                            if (item.type === 'bid') {
                                                message = message.replace(/(.*?) have earned the players rights for (.*?) with a bid amount of (\S+).*/i, (match, team, player, amount) => `${item.customTeams ? 'The ' : ''}${team} have won the rights to ${utils.tagUser(player, guild)} with a bid of ${bid}!`);
                                            } else if (item.type === 'contract') {
                                                message = message.replace(/(.*?) and the (.*?) have agreed to a (\d+) season deal at (\S+) per season/i, (match, player, team, length, amount) => `${item.customTeams ? 'The ' : ''}${team} have signed ${utils.tagUser(player, guild)} to a ${length} season contract worth ${amount} per season!`);
                                            } else if (item.type === 'draft') {
                                                message = message.replace(/The (.*?) have drafted (.*?) (\S+) overall in season (\d+) of the (.*?)/i, (match, team, player, position, season, league) => `${item.customTeams ? 'The ' : ''}${team} have drafted ${util.tagUser(player, guild)} ${position} overall in the ${league} season ${season} draft!`);
                                            }
